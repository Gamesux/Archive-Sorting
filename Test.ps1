$mainfolder = Get-ChildItem 'C:\Users\Monika\Documents\Prüfanweisungen\' | where { $_.PsIsContainer -eq $true } #Grabs only Folders from the main folder and ignores files
foreach ($folder in $mainfolder) {
        ...
        $anweisungpath = 'C:\Users\Monika\Documents\Prüfanweisungen\' + $articlefolder 			#The above specified Article folders path is turned into a Variable for easier use
